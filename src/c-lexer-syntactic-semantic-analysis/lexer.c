...
void AddToken ( const TokenType type ) {
    ...
    /* In this section will be the management for syntactic and semantic analysis */
...
void Lexer ( const char* file_path ) {
    InitLexer(file_path);
    MainLexer();
    CloseLexer();
}
...
void MainLexer ( void ) { /* Lexer structure based on Crafting Interpreters: https://craftinginterpreters.com/scanning.html */
    while ( scanner.peek() != '\0' ) {
        ...
        switch ( c ) {
            ...
            case '-':
                AddToken(scanner.match('-') ?
                    (tBuffer[++current] = scanner.peek(), MINUS_MINUS) :
                    scanner.match('=') ?
                    (tBuffer[++current] = scanner.peek(), MINUS_EQUAL) :
                    scanner.match('>') ?
                    (tBuffer[++current] = scanner.peek(), ARROW) :
                    MINUS);
                break;

            case '+':
                AddToken(scanner.match('+') ?
                         (tBuffer[++current] = scanner.peek(), PLUS_PLUS) :
                         scanner.match('=') ?
                         (tBuffer[++current] = scanner.peek(), PLUS_EQUAL) :
                         PLUS);
                break;

            case '=':
                AddToken(scanner.match('=') ?
                         (tBuffer[++current] = scanner.peek(), EQUAL_EQUAL) :
                         EQUAL);
                break;

            case '<':
                AddToken(scanner.match('=') ?
                         (tBuffer[++current] = scanner.peek(), LESSER_EQUAL) :
                         scanner.match('<') ?
                         (tBuffer[++current] = scanner.peek(), scanner.match('=') ?
                         (tBuffer[++current] = scanner.peek(), LEFT_SHIFT_EQUAL) :
                         LEFT_SHIFT) :
                         LESSER);
                break;
            ...
            case '>':
                AddToken(scanner.match('=') ?
                         (tBuffer[++current] = scanner.peek(), GREATER_EQUAL) :
                         scanner.match('>') ?
                         (tBuffer[++current] = scanner.peek(), scanner.match('=') ?
                         (tBuffer[++current] = scanner.peek(), RIGHT_SHIFT_EQUAL) :
                         RIGHT_SHIFT) :
                         GREATER);
                break;
            ...
            case '/':
                if (scanner.next() == '/' || scanner.next() == '*') {
                    ...
                    AddToken(scanner.match('=') ?
                             (tBuffer[++current] = scanner.peek(), SLASH_EQUAL) :
                             SLASH);
                }
                break;
            ...
            case '*':
                AddToken(scanner.match('=') ?
                         (tBuffer[++current] = scanner.peek(), STAR_EQUAL) :
                         STAR);
                break;

            case '&':
                AddToken(scanner.match('&') ?
                         (tBuffer[++current] = scanner.peek(), AND) :
                         scanner.match('=') ?
                         (tBuffer[++current] = scanner.peek(), BIT_AND_EQUAL) :
                         BIT_AND);
                break;

            case '|':
                AddToken(scanner.match('|') ?
                         (tBuffer[++current] = scanner.peek(), OR) :
                         scanner.match('=') ?
                         (tBuffer[++current] = scanner.peek(), BIT_OR_EQUAL) :
                         BIT_OR);
                break;

            case '^':
                AddToken(scanner.match('=') ?
                         (tBuffer[++current] = scanner.peek(), X_OR_EQUAL) :
                         X_OR);
                break;

            case '!':
                AddToken(scanner.match('=') ?
                         (tBuffer[++current] = scanner.peek(), DIFFERENT) :
                         NOT);
                break;
...
static void Identifiers ( void ) {
    ...
    tBuffer[current] = scanner.peek();
    current++;
    const TokenType type = map.get_token(tBuffer, current);
...
static void Number ( void ) {
    ReadNumber();
    if ( scanner.next() == '.' ) {
        tBuffer[++current] = scanner.advance();
        ReadNumber();
    }
    AddToken(NUMBER);
}
...
static void String ( void ) {
    scanner.advance();
    const unsigned line = scanner.get_line_number();
    while ( scanner.peek() != '"' ) {
...
static void Comments ( void ) {
    ...
    while (1) {
        ...
        if (scanner.peek() == '\0') { error_at("Unterminated commentary at line %d\n", line); }
        scanner.advance();
    }
    scanner.advance();
}

static void Preprocessor ( void ) { /* Just ignores preprocessor instructions */
    current = -1;
    while ( scanner.peek() != '\n' ) scanner.advance();
}